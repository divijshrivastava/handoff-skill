...
Keep `skills/handoff/SKILL.md` the runtime source of truth. Put conditional details in its references and keep the helper dependency-free. `doctor`, `validate`, `read`, and `template` are read-only, as are the viewer's `--once`, `--bar`, and `--codex` modes and the `handoff-tui` launcher. Two paths write the ledger: `apply`, and the live viewer's task move, which `--read-only` disables. Both go through the one compare-and-swap, `swap_ledger`, and any change to it must keep the read, version check, and replacement inside a single held lock; do not add a third write path beside it.
