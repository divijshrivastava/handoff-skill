...
`doctor`, `validate`, `read`, `template`, and `assignments` never write; `apply` and `purge` are the writing subcommands. `assignments` reports entries recorded to an owner with no step checked yet, and reads back the viewer's own move note so a pull can tell work a user handed over from work an agent claimed. It resolves the session's name with `recall_name`, never `claim_name`: a report is not the session asking for a name. It exists because the viewer's move writes the ledger and nothing else, so a running CLI is never pushed the assignment — `/handoff:queue` is its command. It is deliberately not named `nudge`: the channel's `nudge` messages a peer, while this only reads the ledger. `read` returns ledger text *and* its version hash from a single snapshot, because two separate reads bind an audit of old text to a newer version and lose a peer's entry.
...
**One version, generated.** `skills/handoff/SKILL.md` (`metadata.version`) is the only place a version is written by hand. `scripts/sync_manifests.py` generates every manifest under `*-plugin/` from it plus one host table; `--check` writes nothing and fails when a manifest on disk differs, and runs in CI. A release bumps `SKILL.md` and runs the generator. Do not hand-edit a manifest.
...
`skills/handoff/scripts/handoff_channel.py` keeps a durable local inbox and explicit capability reports in gitignored `.handoff/`. Root `hooks/hooks.json` supplies Claude Code session registration, failure reporting, and inbox delivery. A StopFailure hook can report an unavailable model while its CLI remains open; it does not itself authorize takeover or prove child writers stopped. Exhaustion is handled by construction rather than detection: an owner declares a renewal deadline with guard `lease`, any peer releases what expired with `sweep`, and channel `challenge`/`attest` bind a capability proof to a nonce. The channel carries two kinds of message and the kind decides what the recipient owes: a normal message (`send`) asks or tells, while a task message (`assign-task`) assigns work, carries the title and steps the follower records, and cannot be broadcast because an assignment needs one owner. A task message still grants no ownership — `record` is what writes the entry through `swap_ledger`, taking the owner name and harness label from the recording session's own registration, so a message that is missed, duplicated, or read by a dead session never decides who owns work. `record` defaults to a pending entry, which is the mid-task case: a busy follower records the assignment, returns to what it was doing, and picks the new entry up from the ledger afterwards, where `assignments` lists it; `--start` is for a follower that is free now. Recording twice returns the first heading instead of duplicating the entry, and that holds through a crash: the ledger write and the `assignments` row are two steps, so `build` refuses inside the lock when the entry's heading is already in the ledger and the retry writes the missing row instead. The heading it reports is parsed from the entry it built, never read back from the ledger afterwards — `insert_entry` puts new entries first, so a read outside the lock returns whichever peer applied last. The `assignments` table is last in `SCHEMA` and `NEWEST_TABLE` names it, because that constant gates the migration check on connect — leaving it on the previous table is silent, working for new channels and failing every existing one. Channel `nudge` sits below all of it and decides nothing: it is a message asking a silent peer to answer, rate-limited per sender and subject, and `silence` is its read-only companion reporting unacknowledged messages and report age. `silent` describes the record, never the peer, so it is not a takeover timer. Host hooks only write those records earlier; they never decide, because a verdict has to be computable on every harness from the ledger, the channel, and the clock. See `skills/handoff/references/agent-channel.md` for the workflow. All ownership changes still pass through `swap_ledger`.
