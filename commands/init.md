...
1. Claim this session's name with the helper's `name` command, and use it
   verbatim as the owner label of every entry this session writes.
2. Read every applicable repository instruction file.
3. If `HANDOFF.md` exists, read the whole ledger with the helper's `read`
   command so the text and version come from one snapshot. If none exists,
   follow the repository's local instructions; when none are prescribed and
   mutation is authorized, create `HANDOFF.md` from the skill's
   `references/ledger-contract.md`. Never create a parallel ledger under
   another name.
4. Run the read-only `doctor` and `git status`.
