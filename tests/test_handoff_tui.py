...
class SpawnAgentTests(unittest.TestCase):
    """Press N in Agents to pick a CLI and open it in a new terminal."""

    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp.cleanup)
        self.path = Path(self.temp.name) / "HANDOFF.md"
        self.path.write_text("# Handoff\n\n" + entry("Importer"), encoding="utf-8")
    ...
    def test_n_opens_the_agent_picker(self):
        dashboard = self.board()
        with patch.object(tui, "available_agents", return_value=["claude", "codex"]):
            self.press(dashboard, ord("N"))
            self.assertEqual(dashboard.view, "spawn")
            self.assertEqual([row[0] for row in dashboard.rows()], ["claude", "codex"])

    def test_n_with_no_clis_on_path_reports_why(self):
        dashboard = self.board()
        with patch.object(tui, "available_agents", return_value=[]):
            self.press(dashboard, ord("N"))
        self.assertEqual(dashboard.view, "agents")
        self.assertIn("No agent CLIs on PATH", dashboard.message)

    def test_enter_launches_the_selected_agent_and_returns_to_agents(self):
        dashboard = self.board()
        with patch.object(tui, "available_agents", return_value=["claude", "codex"]):
            self.press(dashboard, ord("N"))
            with patch.object(tui, "open_agent", return_value=(0, "Opened claude with the handoff bar in iTerm2.")) as launch:
                self.press(dashboard, 10)
        launch.assert_called_once_with(self.path.resolve().parent, "claude")
        self.assertEqual(dashboard.view, "agents")
        self.assertIn("Opened claude", dashboard.message)

    def test_back_leaves_the_picker_without_launching(self):
        dashboard = self.board()
        with patch.object(tui, "available_agents", return_value=["claude"]):
            self.press(dashboard, ord("N"))
            with patch.object(tui, "open_agent") as launch:
                self.press(dashboard, ord("b"))
        launch.assert_not_called()
        self.assertEqual(dashboard.view, "agents")

    def test_the_agents_footer_advertises_the_new_agent_key(self):
        screen = Screen()
        dashboard = self.board()
        dashboard.draw(screen, FakeCurses)
        self.assertIn("N new agent", screen.lines[screen.height - 1])
