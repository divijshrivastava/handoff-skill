...
class AgentLaunchTests(unittest.TestCase):
    def tearDown(self) -> None:
        keys._PATH_ENRICHED = False

    def test_available_agents_finds_clis_after_enriching_a_bare_path(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            fake.chmod(0o755)
            with patch.object(keys, "agent_path_entries", return_value=[str(bindir)]):
                with patch.dict(os.environ, {"PATH": "/usr/bin:/bin"}, clear=False):
                    self.assertEqual(keys.available_agents(), ["cursor-agent"])

    def test_available_agents_keeps_only_known_clis_on_path(self):
        with patch.object(keys.shutil, "which", side_effect=lambda name: f"/bin/{name}" if name in {"claude", "grok"} else None):
            self.assertEqual(keys.available_agents(), ["claude", "grok"])

    def test_agent_launch_command_uses_the_path_launcher(self):
        with patch.object(keys.shutil, "which", return_value="/usr/local/bin/handoff-tui"):
            command = keys.agent_launch_command(Path("/repo"), "claude", seed="abc")
        self.assertEqual(command,
                         "/usr/local/bin/handoff-tui --root /repo --session-seed abc --with claude")

    def test_open_agent_reports_success(self):
        with patch.object(keys, "open_candidates", return_value=["iterm2"]):
            with patch.object(keys, "agent_launch_command", return_value="handoff-tui --root /repo --with claude"):
                with patch.object(keys.subprocess, "run"):
                    code, message, name = keys.open_agent(Path("/repo"), "claude")
        self.assertEqual(code, 0)
        self.assertIn("claude", message)
        self.assertIn("iTerm2", message)
        self.assertIsNotNone(name)

    def test_open_agent_claims_a_name_before_spawning(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, {"HANDOFF_NAME_CACHE": str(cache)}):
                with patch.object(keys, "open_candidates", return_value=["iterm2"]):
                    with patch.object(keys.subprocess, "run"):
                        code, message, name = keys.open_agent(repo, "cursor-agent", seed="spawn-seed")
            self.assertEqual(code, 0)
